...
#figure(
  ...
  table.cell(colspan: 3, fill: blue.lighten(80%), [*Linguaggio*] ),
    ...
    // [JavaScript],
    // [],
    // [Linguaggio di programmazione interpretato, utilizzato principalmente per lo sviluppo web lato client e server. TypeScript viene compilato in JavaScript.],
  ...
  table.cell(colspan: 3, fill: blue.lighten(80%), [*Strumenti*] ),
    ...
    [Class Transformer],
    [0.5.1],
    [Libreria per trasformare oggetti plain JavaScript in classi di TypeScript e viceversa.],

    [Class Validator],
    [0.14.1],
    [Libreria per validare oggetti basati su decorators di TypeScript.],

    [Reflect Metadata],
    [0.2.2],
    [Libreria necessaria per abilitare le funzionalità di _metadata reflection_#super("G") di TypeScript, richiesta da NestJS.],

    [RxJS],
    [7.8.1],
    [Libreria per programmazione reattiva, utilizzata in NestJS per gestire stream asincroni.],
...
- *Backend:* Il server NestJS è stato sviluppato con un _architettura esagonale_ ed è il responsabile della logica di business e dell'interazione con il modello LLM. Comunica con il frontend esponendo API RESTful e gestisce le richieste provenienti dall'estensione. Utilizza librerie come _Axios_ per effettuare chiamate HTTP e _Class Transformer_ per la serializzazione e deserializzazione degli oggetti. Il server è progettato per essere facilmente estensibile e manutenibile, consentendo l'aggiunta di nuove funzionalità in modo semplice.

Questa struttura è stata scelta per garantire modularità, estensibilità, manutenibilità e facilità di testing.

== Pattern utilizzati
=== Dependency Injection (DI)
Dependency Injection è un pattern di progettazione ampiamente adottato per implementare efficacemente il principio di inversione delle dipendenze (Dependency Inversion Principle). Questo pattern si basa sul fornire a un oggetto (il "dipendente") le dipendenze di cui necessita dall'esterno, invece di lasciare che sia l'oggetto stesso a crearle o a cercarle attivamente al suo interno. Le dipendenze possono essere "iniettate" in vari modi, i più comuni dei quali includono l'iniezione tramite costruttore, l'iniezione tramite metodo setter o l'iniezione tramite interfaccia. L'utilizzo della DI semplifica notevolmente la gestione delle dipendenze, facilita l'isolamento dei componenti per i test unitari (consentendo l'uso di mock o stub) e promuove un'architettura software più pulita e basata sull'interazione tra astrazioni piuttosto che su implementazioni concrete rigidamente accoppiate. Nel progetto è stato utilizzato il pattern di dependency injection tramite il costruttore.

=== Facade

Il pattern Facade è un design pattern strutturale che fornisce un'interfaccia semplice per interagire con un sistema complesso. Aiuta a ridurre la complessità nascondendo i dettagli interni e fornendo un unico punto di accesso.

L'uso del pattern Facade migliora la leggibilità del codice e semplifica l'interazione dei client con il sistema, promuovendo una chiara separazione tra le componenti interne e l'accesso esterno.
...
#figure(
  image("img/ST/frontend minimale 0.2.0.svg"),
  caption: [Diagramma del Frontend]
)
...
L'ordine delle dipendenze è strettamente controllato: ogni strato può dipendere solo dagli strati sottostanti, prevenendo accoppiamenti ciclici e garantendo un'architettura scalabile e facilmente mantenibile.
...
#figure(
  ...
  caption: [Diagramma dello stratto Presentation]
)
...
- *executeFilterCommand*: 
  Gestisce il comando per filtrare i requisiti in base a un termine di ricerca. Se non viene fornito un termine, mostra un input box per richiederlo all'utente.
...
- *executeEditTraceabilityCommand*: 
  Gestisce il comando per modificare un collegamento di tracciabilità. Mostra input box per consentire all'utente di modificare il percorso del file e i numeri di riga.
...
*Descrizione*: Estende la classe `vscode.TreeItem` di VS Code per rappresentare un elemento nell'albero dei requisiti. Ogni istanza rappresenta un requisito o un dettaglio di un requisito nell'interfaccia utente.

*Metodi*:
- *constructor*: 
  Inizializza un nuovo elemento dell'albero dei requisiti con le proprietà specificate.
...
*Descrizione*: Implementa l'interfaccia `vscode.WebviewViewProvider` per fornire una vista webview personalizzata che consente agli utenti di cercare, filtrare e analizzare i requisiti attraverso un'interfaccia grafica.
...
*Descrizione*: Modulo che registra tutti i comandi dell'estensione con l'API di VS Code. Questi comandi possono essere invocati dall'interfaccia utente o da altre parti dell'estensione.
...
=== Application Layer
#figure(
  image("img/ST/Dettaglio Application 0.2.1.svg"),
  caption: [Diagramma dello stratto Application]
)
L'Application Layer funge da intermediario tra il Presentation Layer e il Core Layer. Questo strato coordina le operazioni dell'applicazione, applica la logica specifica dei casi d'uso e gestisce il flusso di dati tra i vari strati. Implementa il pattern Facade per semplificare l'interazione con i servizi sottostanti e organizza la logica in manager specializzati.

==== Interfaccia: IRequirementsService

*Descrizione*: Definisce il contratto per il servizio di gestione dei requisiti. Questa interfaccia stabilisce tutte le operazioni che il Presentation Layer può richiedere all'Application Layer, fornendo una chiara separazione tra i due strati.
...
*Descrizione*: Implementazione dell'interfaccia IRequirementsService che agisce come facade per i vari manager specializzati. Coordina le operazioni tra i manager e fornisce un'interfaccia unificata al Presentation Layer.
...
*Descrizione*: Gestisce l'archiviazione e l'accesso ai dati dei requisiti. Mantiene lo stato dei requisiti e fornisce metodi per accedervi e modificarli.
...
*Descrizione*: Definisce le modalità di ordinamento disponibili per i requisiti.
...
*Descrizione*: Gestisce il filtraggio e l'ordinamento dei requisiti in base a criteri specificati dall'utente.
...
*Descrizione*: Gestisce l'analisi dei requisiti e del codice sorgente. Coordina le chiamate ai servizi esterni per l'analisi e l'elaborazione dei risultati.
...
*Descrizione*: Gestisce le informazioni di tracciabilità tra requisiti e codice sorgente.
...
*Descrizione*: Gestisce lo stato di approvazione dei requisiti.
...
#figure(
  image("img/ST/Dettaglio Core 0.2.1.svg"),
  caption: [Diagramma dello stratto Core]
)
...
*Descrizione*: Definisce il contratto per il servizio di configurazione dell'estensione. Questa interfaccia permette di accedere alle impostazioni di configurazione in modo indipendente dall'implementazione specifica.
...
*Descrizione*: Definisce il contratto per il servizio di importazione ed esportazione di dati in formato CSV. Questa interfaccia permette di gestire le operazioni di I/O relative ai requisiti in modo indipendente dall'implementazione.
...
*Descrizione*: Definisce il contratto per il servizio di lettura dei file. Questa interfaccia permette di accedere al filesystem in modo indipendente dall'implementazione specifica.
...
*Descrizione*: Definisce il contratto per il servizio di parsing del codice sorgente. Questa interfaccia permette di estrarre informazioni strutturali dal codice in modo indipendente dall'implementazione.
...
*Descrizione*: Definisce il contratto per il servizio di comunicazione con l'API dei requisiti. Questa interfaccia permette di effettuare chiamate API in modo indipendente dall'implementazione specifica.
...
*Descrizione*: Definisce i modelli di dati fondamentali utilizzati nell'applicazione per rappresentare requisiti, analisi e tracciabilità.
...
*Descrizione*: Fornisce funzionalità per il calcolo della similarità tra vettori di embedding, utilizzato per l'analisi della tracciabilità.
...
*Descrizione*: Fornisce funzionalità per l'estrazione di blocchi di codice dai file sorgente.
...
#figure(
  image("img/ST/Dettaglio Infrastructure 0.2.1.svg"),
...
*Descrizione*: Implementa l'interfaccia `IRequirementsApiService` per gestire le comunicazioni con l'API di analisi dei requisiti. Questa classe è responsabile di effettuare le chiamate HTTP all'API esterna e di gestire le risposte e gli errori.
...
*Descrizione*: Implementa l'interfaccia `IConfiguration` per gestire l'accesso alle impostazioni di configurazione dell'estensione. Utilizza l'API di configurazione di VS Code per leggere e scrivere le impostazioni.
...
*Descrizione*: Implementa l'interfaccia `ICsvService` per gestire l'importazione e l'esportazione di dati in formato CSV. Questa classe è responsabile della lettura e scrittura di file CSV e della conversione tra il formato CSV e gli oggetti di dominio.
...
*Descrizione*: Implementa l'interfaccia `IFileReaderService` per gestire la lettura dei file dal filesystem. Questa classe è responsabile dell'accesso al filesystem e della lettura dei contenuti dei file.
...
*Descrizione*: Implementa l'interfaccia `IParseCodeService` per gestire il parsing del codice sorgente. Questa classe è responsabile dell'estrazione e dell'analisi del codice sorgente dai file specificati.
...
1. *Separazione delle responsabilità:* La Layered Architecture consente di separare le diverse responsabilità del sistema in moduli distinti, facilitando la comprensione e la manutenibilità del codice. Ogni layer ha compiti specifici e può essere sviluppato e testato in modo indipendente.
...
#figure(
  image("img/ST/Backend - Minimal v0.1.3.svg", width: 92%),
...
=== Domain
#figure(
  image("img/ST/Dettaglio Domain v0.1.3.svg"),
  caption: [Diagramma architetturale del Domain]
)
Il _Domain_ dell'architettura esagonale racchiude tutta la _business logic_ dell'MVP, che include i modelli e i servizi. Il dominio è indipendente dalle tecnologie esterne e può essere testato in modo isolato.

==== Domain Services
Un _service_ del dominio è una classe che utilizza la logica di business per fornire un servizio. I services del dominio sono responsabili dell'implementazione degli inbound port e della gestione delle interazioni tra i models. Nel nostro caso, esiste solo un servizio a livello di dominio.
===== Classe: `RequirementAnalysisService`
*Descrizione*: Questo servizio gestisce l'analisi dei requisiti e del codice, l'embedding e l'interpretazione delle risposte da parte del server di Ollama.
...
*Descrizione*: Modello che rappresenta i risultati di un'analisi dei requisiti. Contiene informazioni legate ai risultati complessivi dell'analisi di un singolo requisito.
...
- *`parseError`*: Indica se si è verificato un errore durante il _parsing_.

===== Interfaccia: `CodeAnalysisModel`
*Descrizione*: Modello che descrive le proprietà dei risultati dell'analisi del codice associato a un requisito, come la qualità del codice, la copertura dei requisiti e i potenziali problemi.
...
*Descrizione*: Modello che descrive la struttura delle risposte delle analisi, includendo i risultati aggregati e i dettagli rilevanti.
...
*Descrizione*: Modello che descrive la struttura della risposta ricevuta dall'API Odi llama.

*Campi*:
- *`response`*: Risposta ricevuta dall'API.

===== Interfaccia: `OllamaRequestModel`
*Descrizione*: Modello che rappresenta una richiesta inviata all'API Ollama.
...
*Descrizione*: Modello che rappresenta una risposta di embedding dall'API Ollama.
...
=== Ports
I _Ports_ sono interfacce che definiscono le operazioni che mettono in comunicazione il domain con l'esterno, come per esempio la comunicazione con Ollama per l'analisi dei requisiti. I _ports_ fungono da contratti tra il dominio e gli adapter, consentendo inoltre di aggiungere o modificare l'implementazione di alcune operazioni senza influire sulla business logic.
==== Inbound Ports (Use Cases)
Gli Inbound Ports rappresentano le operazioni che possono essere invocate dai controller o da altri componenti esterni. Questi _ports_ definiscono le interfacce per le operazioni principali del dominio.
...
==== Outbound Ports
Gli outbound ports rappresentano le interfacce per le operazioni che il dominio può eseguire verso l'esterno, come la comunicazione con servizi esterni. Questi _ports_ definiscono le interfacce per gli adapter che implementano le funzionalità necessarie.

===== Interfaccia: `OllamaApiPort`
*Descrizione*: Interfaccia che fornisce metodi di comunicazione con l'API del server di Ollama.
...
*Descrizione*: Interfaccia per la gestione della configurazione del server API.  

*Metodi*:
- *`getConfig`*: Metodo per recuperare la configurazione del server API.

===== Interfaccia: `JsonParserPort`
*Descrizione*: Interfaccia che offre metodi per il _parsing_ JSON.  
*Metodi*:
- *`parseJson`*: Metodo per deserializzare una stringa JSON in un oggetto.

=== Adapters
#figure(
  image("img/ST/Dettaglio Adapters v0.1.3.svg"),
  caption: [Diagramma archiettturale degli Adapters]
)
Gli Adapters sono implementazioni concrete dei _ports_ che consentono al domain di interagire con il mondo esterno, garantendo che il dominio rimanga isolato e indipendente.
==== Inbound Adapters (Controllers)
I controllers sono componenti che gestiscono le richieste da parte di client vari e inoltrano le operazioni da svolgere ai servizi del dominio. I controller sono implementati come adapter e si interfacciano con i _ports_ per invocare la logica di business.
===== Classe: `RequirementAnalysisController`
Questo adapter è responsabile di ricevere le richieste dall'esterno (ad esempio, dal frontend), validarle e inoltrarle ai servizi del dominio tramite i _ports_. Inoltre, si occupa di formattare le risposte provenienti dal dominio in un formato comprensibile per il client. Infine, esso gestisce gli endpoint HTTP esposti dal backend, in modo da permettere la comunicazione dei requisiti da analizzare da parte del frontend.
...
==== Outbound Adapters
Gli outbound adapters sono implementazioni concrete degli outbound ports che consentono al dominio di interagire con servizi esterni, come API o sistemi di archiviazione. Questi adapter si interfacciano con i _ports_ per invocare le operazioni necessarie.
...
=== Application
#figure(
  image("img/ST/Dettaglio Application v0.1.3.svg"),
...
Modulo specifico per la gestione dell'analisi dei requisiti. Contiene la configurazione dei servizi, dei _ports_ e degli adapter relativi all'analisi dei requisiti. Questo modulo garantisce che tutte le dipendenze necessarie per l'analisi siano correttamente iniettate e configurate.
